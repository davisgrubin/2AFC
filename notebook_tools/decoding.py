"""Notebook helper implementations migrated from `Test_pilot/test_nb_decoding.py`.
...
def neural_decoder_svm(data, labels, neu_time, trial_type, isi, label_names, shuffle = None, bin_data=False, bin_size=3, test_size=0.2, 
    ...
    if save_path is not None:
        report_dir = os.path.join(save_path, 'Figures', 'decoder')
...
def decoder_decision(neural_trials, labels, decoding, indice, l_frames=60, r_frames=120, 
    ...
    if decoding == 'epoch_short':
        ...
        labels_decoder = generate_majority_rare_labels(block_type, trial_type, outcomes=outcome, process_blocks='long', process_outcomes='punish')
