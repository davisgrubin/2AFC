"""Notebook helper implementations migrated from `Test_pilot/test_nb_clustering.py`.
...
try:
    ...
    from notebook_tools.alignment import get_perception_response
import numpy as np
...
class NeuralActivityClustering:
    ...
    def find_optimal_clusters(self, max_clusters=15, cv_folds=5):
        ...
        if n_samples < 3:
            raise ValueError('Need at least 3 samples for clustering cross-validation.')
        effective_folds = max(2, min(cv_folds, n_samples))
        kf = KFold(n_splits=effective_folds, shuffle=True, random_state=self.random_state)
...
def clustering_GMM(neural_trials, labels, save_path = None, pooling = False):
    ...
    if pooling:
        neural_trials_list = neural_trials if isinstance(neural_trials, list) else [neural_trials]
